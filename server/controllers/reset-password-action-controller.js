 ...
 */
@httpController()
export default class ResetPasswordActionController extends HttpBaseController {
    ...
    postPasswordReminder(email) {
        ...
        return new Promise((resolve, reject) => {
            ...
            self.context.model('User').where('name').equal(email).silent().getItem().then((user)=> {
                ...
                return self.context.model('ActionStatusType').where('alternateName').equal('ActiveActionStatus').getItem().then((res)=> {
    ...
    postPasswordReset(code, newPassword, confirmPassword) {
        const self = this;
        return new Promise((resolve) => {
            self.context.model('ActionStatusType').where('alternateName').equal('ActiveActionStatus').getItem().then((actionStatus) => {
                return self.context.model('ResetPasswordAction')
                    .where('url').equal(`/password/reset?code=${code}`)
                    .and('actionStatus/alternateName').equal('ActiveActionStatus')
